...
1. 实现Graph Transformation和基本的搜索策略（DFS/BFS），使得对于给定的问题模型和问题实例，可以通过图变换得到一个满足目标的实例，同时可以判断无解的情况
2. 采用给定的模型来描述某一个问题，问题的类型不限，但应该对于搜索具有一定的复杂性，给出问题的模型，以及若干复杂程度不等的实例
